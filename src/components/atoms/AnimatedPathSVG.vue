...
const logoLengths = computed(() => {
  console.log(
    "logopaths.value (comp)"
    // logopaths?.value?.map((lp) => lp?.getTotalLength())
  );
  let paths = Array.from(logopaths?.value || []);
  console.log("paths", paths);
  return paths?.map((lp) => lp?.getTotalLength());
  // return logopaths?.value; //?.map((lp) => lp.getTotalLength());
  // return [];
});
</script>
<style>
#logo {
  ...
  /* animation: glow 0.5s ease forwards 3.5s; */
}
...
@keyframes fill {
  ...
  to {
    fill: white;
    /* text-shadow: 2rem 2rem blue; */
    filter: drop-shadow(3px 10px 2px rgb(25 25 140 / 0.7));
  }
}
</style>
